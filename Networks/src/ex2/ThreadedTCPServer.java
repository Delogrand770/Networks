...
/**
 * A Multithreaded TCP Server (Can Handle Multiple Simultaneous Clients)
 * @author CS467
 */
public class ThreadedTCPServer 
{
    ...
    public ThreadedTCPServer(Socket s) throws IOException, UnknownHostException
    {
        sock       = s;
    ...
    {   
        ...
        while (true)
        {
            ThreadedTCPServer server = new ThreadedTCPServer(listenSock.accept());
        	server.getRequest();
        	server.process();
        	server.sendResponse();
        	server.close();
